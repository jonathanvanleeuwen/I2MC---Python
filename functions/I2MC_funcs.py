...
def windowedInterpolate(xpos, ypos, missing, notAllowed, dev=False):
    """
    ...
    notMissing = np.invert(missing)
    newX = np.arange(len(xpos))
    
    # Interpolate x values
    interpolater = interp.PchipInterpolator(newX[notMissing],xpos[notMissing])
    xi = interpolater(newX)
    xi[notAllowed] = np.nan
    
    # Interpolate y values
    interpolater = interp.PchipInterpolator(newX[notMissing],ypos[notMissing])
    yi = interpolater(newX)
    yi[notAllowed] = np.nan
...
def I2MC(gazeData, options = {}):
    ...
    missingn = findInterpWins(xpos, ypos, missing, par['windowtimeInterp'], par['edgeSampInterp'], par['freq'], par['maxdisp'])
    if q2Eyes:
        llmissn = findInterpWins(data['L_X'], data['L_Y'], llmiss, par['windowtimeInterp'], par['edgeSampInterp'], par['freq'], par['maxdisp']);
        rrmissn = findInterpWins(data['R_X'], data['R_Y'], rrmiss, par['windowtimeInterp'], par['edgeSampInterp'], par['freq'], par['maxdisp']);
    
    # Use Steffen interpolation and replace values
    print('\tReplace interpolation windows with Steffen interpolation')
    xpos, ypos= windowedInterpolate(xpos, ypos, missing, missingn, par['dev_interpolation'])
    if q2Eyes:
        llx, lly = windowedInterpolate(data['L_X'], data['L_Y'], data['left_missing'], llmissn, par['dev_interpolation'])
        rrx, rry = windowedInterpolate(data['R_X'], data['R_Y'], data['right_missing'], rrmissn, par['dev_interpolation'])       
